...
static const struct usb_endpoint_descriptor cp_endp[] = {{
    .bLength = USB_DT_ENDPOINT_SIZE,
    .bDescriptorType = USB_DT_ENDPOINT,
    .bEndpointAddress = 0x83, // Interrupt Endpoint
...
static const struct usb_endpoint_descriptor data_endp[] = {{
    .bLength = USB_DT_ENDPOINT_SIZE,
    .bDescriptorType = USB_DT_ENDPOINT,
    .bEndpointAddress = 0x01, // Out Endpoint (to device)
    ...
    .bLength = USB_DT_ENDPOINT_SIZE,
    .bDescriptorType = USB_DT_ENDPOINT,
    .bEndpointAddress = 0x82, // In Endpoint (to PC)
    .bmAttributes = USB_ENDPOINT_ATTR_BULK,
    .wMaxPacketSize = 64,
    .bInterval = 1,
}};
...
static const struct {
    ...
    struct usb_cdc_union_descriptor union_descr;
    ...
    .union_descr = { .bFunctionLength = sizeof(struct usb_cdc_union_descriptor), .bDescriptorType = CS_INTERFACE, .bDescriptorSubtype = USB_CDC_TYPE_UNION, .bControlInterface = 0, .bSubordinateInterface0 = 1 }
};
...
static enum usbd_request_return_codes cdc_control_request(usbd_device *usbd_dev, struct usb_setup_data *req, uint8_t **buf, uint16_t *len,
    void (**complete)(usbd_device *usbd_dev, struct usb_setup_data *req)) {
    (void)complete; (void)buf; (void)usbd_dev;
    switch (req->bRequest) {
        case USB_CDC_REQ_SET_CONTROL_LINE_STATE: return USBD_REQ_HANDLED;
        case USB_CDC_REQ_SET_LINE_CODING: if (*len < sizeof(struct usb_cdc_line_coding)) return USBD_REQ_NOTSUPP; return USBD_REQ_HANDLED;
    }
...
static void cdc_set_config(usbd_device *usbd_dev, uint16_t wValue) {
    ...
    usbd_register_control_callback(usbd_dev, USB_REQ_TYPE_CLASS | USB_REQ_TYPE_INTERFACE, USB_REQ_TYPE_TYPE | USB_REQ_TYPE_RECIPIENT, cdc_control_request);
}
...
int usb_printf(const char *format, ...) {
    ...
    while (sent < len) {
        ...
        while (usbd_ep_write_packet(usbd_dev, 0x82, &buf[sent], chunk) == 0);
        sent += chunk;
    }
    return len;
}
...
int main(void) {
    ...
    while (1) {
        /* Poll the USB stack as often as possible */
        usbd_poll(usbd_dev);

        if (fusb_event_pending) {
            fusb_handle_irq(); // Now internal calls use usb_printf!
            fusb_event_pending = false;
        }
    }
}

static const char *usb_strings[] = { "MyProject", "PD Sniffer", "SN-001" };
